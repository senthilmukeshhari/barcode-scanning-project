from entry_exit_app.models import Department, Student, Lab
...
def generate_students(initial_value, n, department, section, admission_year):
    n = initial_value + n
...
def generate_labs():
    for i in range(i, 2):
        ...
        if lab_obj.exists():
            lab = Lab.objects.create(name=name, department=department, in_charge=in_charge)
            Lab.save()
        else:
